...
func waitVeleroReady(ctx context.Context, namespace string, useNodeAgent bool) error {
	...
	if useNodeAgent {
		...
		err := wait.PollImmediate(5*time.Second, 1*time.Minute, func() (bool, error) {
			stdout, stderr, err := velerexec.RunCommand(exec.CommandContext(ctx, "kubectl", "get", "daemonset/velero-node-agent",
				"-o", "json", "-n", namespace))
			if err != nil {
				return false, errors.Wrapf(err, "failed to get the velero-node-agent daemonset, stdout=%s, stderr=%s", stdout, stderr)
			}
			daemonset := &apps.DaemonSet{}
			if err = json.Unmarshal([]byte(stdout), daemonset); err != nil {
				return false, errors.Wrapf(err, "failed to unmarshal the velero-node-agent daemonset")
			}
		...
		if err != nil {
			return errors.Wrap(err, "fail to wait for the velero-node-agent ready")
		}
	}
