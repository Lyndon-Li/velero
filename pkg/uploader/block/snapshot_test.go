...
func TestLoadObjectFromSnapshot(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			setupMocks: func(repo *udmrepomocks.BackupRepo) {
				repo.On("ReadMetadata", mock.Anything, udmrepo.ID("root-id")).
					Return((*udmrepo.Metadata)(nil), errors.New("read error"))
			},
			expectedErrStr: "error readding snapshot metadata",
		},
		{
		...
		{
			name: "success returns subobject ID",
			snapshot: &udmrepo.Snapshot{
				RootObject:  udmrepo.ObjectMetadata{ID: "root-id"},
				Description: "test-snap",
			},
