...
func (r *BackupMicroService) RunCancelableDataPath(ctx context.Context) (string, error) {
	...
	callbacks := datapath.Callbacks{
		OnCompleted: r.OnPvbCompleted,
		OnFailed:    r.OnPvbFailed,
		OnCancelled: r.OnPvbCancelled,
		OnProgress:  r.OnPvbProgress,
	}
	...
	if err := fsBackup.StartBackup(r.sourceTargetPath, pvb.Spec.UploaderSettings, &datapath.FSBRStartParam{
		RealSource:     GetRealSource(pvb.Spec.Pod.Namespace, pvb.Spec.Pod.Name, pvb.Spec.Volume),
		ParentSnapshot: "",
		ForceFull:      false,
		Tags:           tags,
	}); err != nil {
...
func (r *BackupMicroService) OnPvbCompleted(ctx context.Context, namespace string, pvbName string, result datapath.Result) {
...
func (r *BackupMicroService) OnPvbFailed(ctx context.Context, namespace string, pvbName string, err error) {
	log := r.logger.WithField("pvb", pvbName)
...
func (r *BackupMicroService) OnPvbCancelled(ctx context.Context, namespace string, pvbName string) {
	log := r.logger.WithField("pvb", pvbName)
...
func (r *BackupMicroService) OnPvbProgress(ctx context.Context, namespace string, pvbName string, progress *uploader.Progress) {
	log := r.logger.WithFields(logrus.Fields{
		"pvb": pvbName,
	})
...
func (r *BackupMicroService) cancelPodVolumeBackup(pvb *velerov1api.PodVolumeBackup) {
	...
	if fsBackup == nil {
		r.OnPvbCancelled(r.ctx, pvb.GetNamespace(), pvb.GetName())
	} else {
		fsBackup.Cancel()
	}
}
