	...
	if dd.Spec.Cancel {
		if spotted, found := r.cancelledDataDownload[dd.Name]; !found {
			...
			if time.Since(spotted) > delay {
				...
				if r.tryCancelAcceptedDataDownload(ctx, dd, "") {
...
func (r *DataDownloadReconciler) tryCancelAcceptedDataDownload(ctx context.Context, dd *velerov2alpha1api.DataDownload, message string) bool {
...
func (r *DataDownloadReconciler) SetupWithManager(mgr ctrl.Manager) error {
	gp := kube.NewGenericEventPredicate(func(object client.Object) bool {
		...
		return (dd.Status.Phase == velerov2alpha1api.DataDownloadPhaseAccepted || dd.Spec.Cancel)
	})
