...
func (kow *kopiaObjectWriterEx) writeObject(objName string, p []byte) (object.ID, error) {
	...
	if writer == nil {
		return object.EmptyID, errors.Errorf("error openning writer for %s", objName)
	}
	...
	if err != nil {
		return object.EmptyID, errors.Wrapf(err, "error writting for %s", objName)
	}
...
func (kow *kopiaObjectWriterEx) writeObjectAsync(objName string, entryID int, p []byte) {
	if kow.asyncWritesSem == nil {
		if err := kow.writeObjectSync(objName, entryID, p); err != nil {
			kow.saveWriteError(errors.Wrapf(err, "error writting object for %s", objName))
		}
	} else {
		kow.asyncWritesSem <- struct{}{}
		kow.asyncWritesGroup.Add(1)
...
func (kow *kopiaObjectWriterEx) saveWriteError(err error) error {
	if err != nil {
