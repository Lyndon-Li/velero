...
// DaemonSetIsReady will poll the kubernetes API server to ensure the velero-node-agent daemonset is ready, i.e. that
...
func DaemonSetIsReady(factory client.DynamicFactory, namespace string) (bool, error) {
	...
	err = wait.PollImmediate(time.Second, time.Minute, func() (bool, error) {
		unstructuredDaemonSet, err := c.Get("velero-node-agent", metav1.GetOptions{})
		if apierrors.IsNotFound(err) {
			return false, nil
		} else if err != nil {
