...
type GenericRestoreExposer interface {
	...
	Expose(context.Context, corev1.ObjectReference, string, string, map[string]string, time.Duration, bool) error
...
func (e *genericRestoreExposer) Expose(ctx context.Context, ownerObject corev1.ObjectReference, targetPVCName string, sourceNamespace string, hostingPodLabels map[string]string, timeout time.Duration, enableProfile bool) error {
	...
	restorePod, err := e.createRestorePod(ctx, ownerObject, targetPVC, timeout, hostingPodLabels, selectedNode, enableProfile)
	if err != nil {
...
func (e *genericRestoreExposer) createRestorePod(ctx context.Context, ownerObject corev1.ObjectReference, targetPVC *corev1.PersistentVolumeClaim,
	operationTimeout time.Duration, label map[string]string, selectedNode string, enableProfile bool) (*corev1.Pod, error) {
	restorePodName := ownerObject.Name
	restorePVCName := ownerObject.Name
	...
	args := []string{
		...
		fmt.Sprintf("--enable-profile=%v", enableProfile),
	}
