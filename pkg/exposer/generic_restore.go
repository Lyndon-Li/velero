...
func (e *genericRestoreExposer) RebindVolume(ctx context.Context, ownerObject corev1api.ObjectReference, targetPVCName string, targetNamespace string, timeout time.Duration) error {
	...
	if err != nil {
		return errors.Wrapf(err, "error rebinding PV for target PVC %s", targetPVCName)
	}

	curLog.WithField("rebind PV", rebindPV.Name).Info("Rebind PV is created")

	err = kube.EnsureDeletePV(ctx, e.kubeClient.CoreV1(), retained.Name, timeout)
	if err != nil {
	...
	_, err = kube.WaitPVBound(ctx, e.kubeClient.CoreV1(), rebindPV.Name, targetPVC.Name, targetPVC.Namespace, timeout)
	if err != nil {
