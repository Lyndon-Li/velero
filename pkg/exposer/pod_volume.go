...
type PodVolumeExposer interface {
	...
	GetExposed(context.Context, corev1.ObjectReference, client.Client, string) (*ExposeResult, error)
...
func (e *podVolumeExposer) GetExposed(ctx context.Context, ownerObject corev1.ObjectReference, nodeClient client.Client, nodeName string) (*ExposeResult, error) {
	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			...
			return nil, errors.Wrapf(err, "error to get hosting pod %s", hostingPodName)
		}
	}

	curLog.WithField("pod", pod.Name).Infof("Hosting pod is in running state in node %s", pod.Spec.NodeName)
