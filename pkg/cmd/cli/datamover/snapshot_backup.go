...
func NewSnapshotBackupCommand(f client.Factory) *cobra.Command {
	logLevelFlag := logging.LogLevelFlag(logrus.InfoLevel)
	formatFlag := logging.NewFormatFlag()

	command := &cobra.Command{
		Use:    "snapshot-backup",
		Short:  "Run the velero snapshot backup data path",
		Long:   "Run the velero snapshot backup data path",
		Hidden: true,
		Run: func(c *cobra.Command, args []string) {
			...
			logger.Infof("Starting Velero snapshot backup data path %s (%s)", buildinfo.Version, buildinfo.FormattedGitSHA())

			f.SetBasename(fmt.Sprintf("%s-%s", c.Parent().Name(), c.Name()))
			s, err := newSnapshotBackupper(f, args[0], logger)
			cmd.CheckError(err)
...
func newSnapshotBackupper(factory client.Factory, request string, logger logrus.FieldLogger) (*snapshotBackupper, error) {
