...
func NewBackupCommand(f client.Factory) *cobra.Command {
	...
	command := &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			if err != nil {
				exitWithMessage(logger, false, "Failed to create pod volume backup, %v", err)
			}
...
func newPodVolumeBackup(logger logrus.FieldLogger, factory client.Factory, config pvbConfig) (*podVolumeBackup, error) {
...
func (s *podVolumeBackup) runDataPath() {
	...
	if err != nil {
		dpService.Shutdown()
		s.cancelFunc()
		funcExitWithMessage(s.logger, false, "Failed to run data path service for DataUpload %s: %v", s.config.pvbName, err)
		return
	}

	s.logger.WithField("du", s.config.pvbName).Info("Data path service completed")

	dpService.Shutdown()

	s.logger.WithField("du", s.config.pvbName).Info("Data path service is shut down")
