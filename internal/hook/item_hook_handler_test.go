...
func TestHandleRestoreHooks(t *testing.T) {
	testCases := []struct {
		...
		{
			name: "should preserve pod-volume-wait init container when it is the only existing init container",
			podInput: corev1api.Pod{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "app1",
					Namespace: "default",
				...
				Spec: corev1api.PodSpec{
					InitContainers: []corev1api.Container{
						*builder.ForContainer("pod-volume-wait", "bus-box").
		...
		{
			name: "should preserve pod-volume-wait init container when it exits with other init containers",
			podInput: corev1api.Pod{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "app1",
					Namespace: "default",
