...
func (i *InitContainerRestoreHookHandler) HandleRestoreHooks(
	...
	// If this pod had pod volumes backed up, then we want to the pod volumes restored prior to
	...
	// So if there is a "pod-volume-wait" init container already on the pod at index 0, we'll preserve that and run
